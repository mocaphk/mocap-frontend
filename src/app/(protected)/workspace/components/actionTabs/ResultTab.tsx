...
function ResultBar({
    ...
    return (
        <Box className="pb-1">
            ...
            <Collapse
                in={allowEditOrCreate?(!result.isCorrect && open):(!result.isHidden && !result.isCorrect && open)}
                timeout="auto"
                unmountOnExit
            >
        ...
        <List className="w-full" aria-label="mailbox folders">
            {allowEditOrCreate
                ? filteredResults?.map((result, index) => (
                      <ResultBar key={index} result={result} index={index} allowEditOrCreate={allowEditOrCreate}/>
                  ))
